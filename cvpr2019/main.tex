...
\begin{abstract}
    ...
    In this work, we formulate the multi-image matching problem as a graph embedding problem then use a Graph Convolutional Network to learn an appropriate embedding function for aligning image features. % How to talk about the universe of features here without going too much into the weeds??
...
Deep learning has revolutionized how image features are computed \cite{yi2016lift}, and so we would like to leverage this to compute better and more robust multi-view scene features.
Graph Convolutional Networks \cite{kipf2016semi} also show promise using learning in a distributed setting.
Thus deep learning seems a promising tool to help solve this problem.
...
% Using features designed to deal with these condfounding factors improves robustness to them, and learning may improve robustness even further.
...
The proposed method works directly on the correspondence graph, which is agnostic to how the correspondnces were computed, thus allowing the algorithm to work in a broad class of environments.
To the best of our knowledge this work is the first to apply deep learning to the multi-view feature matching problem.
We use a unsupervised loss, the cycle consistency loss, to train the network, and thus we avoid the difficulty of expensive hand labelling.
...
Our simple network already has promising results compared to previous optimization based baselines.
...
To the best of our knowledge, we are the first work to use neural networks for multi-image matching.
...
\end{center}
  \caption{
    ...
    We can get the points to have a cycle consistent embedding by mapping each feature in each image to the one-hot vector of its corresponding 3D point.
    While there can be many features, there are many fewer 3D points and thus this corrsponds to a low rank factorization of the corrspondence matrix.
...
Note that because of this formulation, we can determine our embeddings only up to a rotation on the right, as
\begin{equation}
\mat{E}R(\mat{E}R)^\top
...
% MatchALS \cite{zhou2015multi} 400 Iterations              & 0.007 $\pm$ 0.003 & 0.007 $\pm$ 0.003 & 0.007 $\pm$ 0.003 \\ \hline
% PGDDS \cite{leonardos2016distributed} 200 Iterations      & 0.013 $\pm$ 0.002 & 0.005 $\pm$ 0.002 & 0.005 $\pm$ 0.002 & 0.039 $\pm$ 0.039 \\ \hline
\begin{table*}
\begin{center}
\begin{tabular}{|l|c|c|c|}
\hline
Method (3 Views)                                          & $L_1$ Loss                          & $L_2$ Loss                          & Run Time (sec) (TODO: Fill out)    \\
\hline\hline
MatchALS \cite{zhou2015multi} 15 Iterations               & 0.052 $\pm$ 0.003                   & 0.010 $\pm$ 0.002                   & 0.010 $\pm$ 0.002 \\ \hline
MatchALS \cite{zhou2015multi} 25 Iterations               & 0.045 $\pm$ 0.007                   & 0.009 $\pm$ 0.003                   & 0.009 $\pm$ 0.003 \\ \hline
MatchALS \cite{zhou2015multi} 50 Iterations               & 0.016 $\pm$ 0.008                   & 0.007 $\pm$ 0.003                   & 0.007 $\pm$ 0.003 \\ \hline
MatchALS \cite{zhou2015multi} 100 Iterations              & 0.007 $\pm$ 0.003                   & 0.007 $\pm$ 0.003                   & 0.007 $\pm$ 0.003 \\ \hline
PGDDS \cite{leonardos2016distributed} 15 Iterations       & 0.016 $\pm$ 0.002                   & 0.006 $\pm$ 0.002                   & 0.006 $\pm$ 0.002 \\ \hline
PGDDS \cite{leonardos2016distributed} 50 Iterations       & 0.013 $\pm$ 0.002                   & 0.005 $\pm$ 0.002                   & 0.005 $\pm$ 0.002 \\ \hline
PGDDS \cite{leonardos2016distributed} 100 Iterations      & 0.013 $\pm$ 0.002                   & 0.005 $\pm$ 0.002                   & 0.005 $\pm$ 0.002 \\ \hline
Spectral                                                  & 0.054 $\pm$ 0.005                   & 0.018 $\pm$ 0.004                   & 0.018 $\pm$ 0.004 \\ \hline 
\textbf{GNN, 12 Layers (ours)}                            & \textbf{0.025} $\pm$ \textbf{0.003} & \textbf{0.016} $\pm$ \textbf{0.003} & \textbf{0.016} $\pm$ \textbf{0.003} \\
\hline \hline\hline
Method (4 Views)                                          & $L_1$ Loss                          & $L_2$ Loss                          & Run Time (sec) (TODO: Fill out)   \\
\hline\hline
MatchALS \cite{zhou2015multi} 15 Iterations               & 0.052 $\pm$ 0.003                   & 0.010 $\pm$ 0.002                   & 0.010 $\pm$ 0.002 \\ \hline
MatchALS \cite{zhou2015multi} 25 Iterations               & 0.045 $\pm$ 0.007                   & 0.009 $\pm$ 0.003                   & 0.009 $\pm$ 0.003 \\ \hline
MatchALS \cite{zhou2015multi} 50 Iterations               & 0.016 $\pm$ 0.008                   & 0.007 $\pm$ 0.003                   & 0.007 $\pm$ 0.003 \\ \hline
...
We have shown a novel method for training feature matching using GCNs and an unsupervised cycle consistency loss.
...
Moreover we plan to focus on making this method work in a distributed setting, which we could do with very small changes to the current approach.
