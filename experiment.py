...
def experiment(opts, network, index):
  ...
  if opts.debug_plot:
    ...
    plt.show()
    # u, s, v = np.linalg.svd(soutput)
    # newemb = myutils.dim_normalize(u[:,:len(s)])
    # svdsim = np.dot(newemb, newemb.T)
    # fig, (ax0, ax1, ax2) = plt.subplots(nrows=1, ncols=3)
    # im0 = ax0.imshow(newemb)
    # im1 = ax1.semilogy(s)
    # im2 = ax2.imshow(v)
    # plt.show()
    # newemb_slim = newemb[::3,:]
    # fig, (ax0, ax1, ax2) = plt.subplots(nrows=1, ncols=3)
    # im0 = ax0.imshow(newemb_slim)
    # im1 = ax1.imshow(np.dot(newemb,newemb_slim.T))
    # im2 = ax2.imshow(np.dot(newemb_slim,newemb_slim.T))
    # plt.show()
    # # randemb
    # u, s, v = np.linalg.svd(rand)
    # randemb = myutils.dim_normalize(u[:,:len(s)])
    # rsvdsim = np.dot(randemb, randemb.T)
    # fig, (ax0, ax1, ax2) = plt.subplots(nrows=1, ncols=3)
    # im0 = ax0.imshow(svdsim)
    # im1 = ax1.imshow(lsim)
    # im2 = ax2.imshow(rsvdsim)
    # plt.show()
    # print(np.linalg.norm(svdsim-lsim))
    # sys.exit()
    # im2 = ax2.imshow(rand)
    # fig.colorbar(im1, ax=ax1)
    # fig.colorbar(im2, ax=ax2)
